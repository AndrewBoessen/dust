defmodule Mesh do
  ...
  def request_key_from_peer(peer_address) do
    Logger.info("Mesh: requesting master key from #{peer_address}")

    case Bridge.request_key(peer_address) do
      {:ok, key} ->
        :ok = KeyStore.set_key(key)
        Logger.info("Mesh: master key received and stored")
        serve_local_key()

      {:error, reason} ->
        Logger.warning("Mesh: failed to get key from #{peer_address}: #{inspect(reason)}")
        {:error, reason}
    end
  end
  ...
  def serve_local_key do
    case KeyStore.get_key() do
      {:ok, key} ->
        Logger.info("Mesh: advertising master key to peers")
  ...
  defp fetch_key_from_peers do
    ...
    if peers == [] do
      Logger.info("Mesh: no seed peers configured, generating new master key")
      ...
      case result do
        :ok -> :ok
        {:error, _reason} ->
          Logger.warning("Mesh: could not reach any seed peer, using locally generated key")
          serve_local_key()
      end
    end
  end

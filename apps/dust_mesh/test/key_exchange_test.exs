defmodule Dust.Mesh.KeyExchangeTest do
  use ExUnit.Case, async: false

  import Mox

  alias Dust.Core.KeyStore
  alias Dust.Mesh.KeyExchange
  ...
  describe "bootstrap_key/0" do
    test "serves key when KeyStore already has one" do
      ...
      expect(Dust.Bridge.Mock, :serve_key, fn received_key ->
  ...
  describe "serve_local_key/0" do
    ...
    test "returns error when Bridge.serve_key fails" do
      ...
      expect(Dust.Bridge.Mock, :serve_key, fn _key ->
  ...
  describe "request_key_from_peer/1" do
    test "fetches key from peer, stores it, and starts serving" do
      ...
      expect(Dust.Bridge.Mock, :request_key, fn "10.0.0.1" ->
        {:ok, peer_key}
      end)
    ...
    test "returns error when peer is unreachable" do
      ...
      expect(Dust.Bridge.Mock, :request_key, fn "10.0.0.99" ->
  ...
  describe "bootstrap_key/0 with seed peers" do
    setup do
      ...
      previous = Application.get_env(:dust_mesh, :seed_peers, [])
      Application.put_env(:dust_mesh, :seed_peers, ["10.0.0.1", "10.0.0.2"])
      on_exit(fn -> Application.put_env(:dust_mesh, :seed_peers, previous) end)
      :ok
    end
    ...
    test "falls back to second peer when first fails" do
      ...
      expect(Dust.Bridge.Mock, :request_key, fn "10.0.0.2" ->
        {:ok, peer_key}
      end)
